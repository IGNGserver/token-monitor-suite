...
els.collectionCadenceInput?.addEventListener('change', async () => {
  ...
  await saveSettings({
    collectionMode: value === 'live' ? 'live' : smart ? 'smart' : 'interval',
    collectionIntervalMs: value === 'live'
      ? Number(state.settings.collectionIntervalMs || 300000)
      : smart ? Number(state.settings.collectionIntervalMs || 600000) : Number(value)
  });
});
