    ...
    <div class="settings-layout">
      <form class="panel settings-form" data-web-settings-form data-draft-key="preferences">
        ...
        <div class="form-grid">
          ...
          ${desktopHost ? '' : `<label class="field"><span>${tr('desktop.settings.reduceMotion')}</span><select name="reduceMotion">${settingsOptionList([['system', tr('desktop.settings.motionSystem')], ['on', tr('desktop.settings.motionOn')], ['off', tr('desktop.settings.motionOff')]], appState().prefs.reduceMotion || 'system')}</select></label>`}
        ...
        <div class="drawer-actions"><fluent-button appearance="primary" type="submit" class="primary-btn">${escapeHtml(tr('settings.savePage'))}</fluent-button>${desktopHost ? '' : `<fluent-button appearance="transparent" type="button" class="ghost-btn" data-web-signout>${escapeHtml(tr('settings.signOut'))}</fluent-button>`}</div>
      </form>
