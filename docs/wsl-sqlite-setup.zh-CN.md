...
- 另一种方式：让 WSL agent 采集全部 WSL 工具，然后在 Windows 桌面端的 **设置 → 采集** 中关闭 **扫描运行中的 WSL 发行版用量**。
...
- **总量重复**：缩小 `TOKEN_MONITOR_CLIENTS` 的范围；如果 agent 负责全部 WSL 工具，则关闭 Windows 桌面端的内建 WSL 扫描。
- **WSL 状态仍报告无用量**：该状态只描述 Windows 侧自己的 `\\wsl# WSL SQLite 用量配置指南
...
- **请求被代理拦截**：把 Windows 主机 IP 加入 `NO_PROXY` 与 `no_proxy`，或为 agent 进程取消代理环境变量。
- **总量重复**：缩小 `TOKEN_MONITOR_CLIENTS` 的范围；如果 agent 负责全部 WSL 工具，则关闭 Windows 桌面端的内建 WSL 扫描。
 扫描。WSL agent 会作为另一台同步设备出现，并作为这些 SQLite 工具的权威来源。
