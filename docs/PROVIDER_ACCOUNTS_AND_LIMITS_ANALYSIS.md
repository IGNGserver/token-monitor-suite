...
> **复核说明（当前工作树）**：本文最初写于 Hub 网页端仍是独立实现的时期，其中引用的
> `src/hub/web/js/{app,data,i18n}.js` 与 `src/hub/web/css/app.css` 已在 `5977c28`
> （抽取 shared UI）中被删除，桌面端 widget 渲染层也已在 `7f2df5b` 移除。所有 UI 相关的
> 「涉及文件」已重链到 `src/shared-ui/` 下的现存实现。provider 端点、凭据字段与
> `LIMIT_PROVIDER_IDS` / `HUB_ACCOUNT_PROVIDERS` 的数量均已按代码逐一校正。

## 一、背景与架构定位

在本项目最新的两层/纯 Docker Compose 架构下：
- **客户端（桌面应用 / Headless Agent）**：原则上不再在本地探测并上传私密额度凭据，限额与配额的权威中心收敛至 **Docker Compose Hub**。
...
权威集合 `LIMIT_PROVIDER_IDS` 包含 27 个 Provider，Hub 手动添加列表 `HUB_ACCOUNT_PROVIDERS` 包含 26 个（仅 `kiro` 被排除，原因见 §17）。下面对各家服务商的数据获取真实可行性进行逐一审计：

### 1. Codex (`codex`) —— ✅ **真实可行**
- **数据源**：
  - OpenAI 官方 CLI 接口：`https://chatgpt.com/backend-api/wham/usage`（`limitCollector.js` 的 `CODEX_CHATGPT_BASE_URL` + `CODEX_USAGE_PATH`）。OAuth 令牌走 `https://auth.openai.com/oauth/token`；`api.openai.com` 只是本地 `auth.json` 的 key 命名空间，不是取数主机。
...
- **数据源**：
  - 远程 Cloud API：`https://cloudcode-pa.googleapis.com`（沙箱区为 `https://daily-cloudcode-pa.sandbox.googleapis.com`），通过 Google OAuth 换取的 Access Token 调用 `RetrieveUserQuotaSummary` 拉取配额窗口。
  - RPC 模式：当提供本地/内网语言服务端的 `endpoint` 和 `csrfToken` 时，通过 gRPC-Web/HTTP 调用同一 RPC。`antigravity.google` 只是 OAuth 回调来源，不是取数端点。
...
- **数据源**：
  - `https://openrouter.ai/api/v1/key`（读取 Key 维度的每日、每周、每月及终身消耗和上限）。
...
- **数据源**：`https://api.github.com/copilot_internal/user`（企业托管时替换 host 段），配合 `https://api.github.com/user`。
- **配额数据**：月度套餐配额、Premium 额度、重置日期。
- **实测可行性**：只要 Token（例如 `ghu_...`）拥有 Copilot 内部权限即可正常获取。

### 8. MiniMax (`minimax`) —— ✅ **真实可行**
- **数据源**：`https://api.minimax.io/v1/token_plan/remains` 与 `https://api.minimax.io/v1/api/openplatform/coding_plan/remains`（国内区对应 `https://api.minimaxi.com/...`）。
...
- **数据源**：
  - Kimi Code API: `https://api.kimi.com/coding/v1`（API Key 鉴权）。
  - Kimi Web: `https://www.kimi.com/apiv2` 的用量与会员状态接口（Web Access Token 鉴权，refresh 走 `https://auth.kimi.com/api/oauth/token`）。
- **配额数据**：5 小时并发窗口、每周额度、共享月度会员包。
- **实测可行性**：代码成熟，合并展示效果好。

### 11. 智谱 GLM / GLM Team (`zai` / `zaiteam`) —— ✅ **真实可行**
- **数据源**：`/api/monitor/usage/quota/limit` 与 `/api/biz/subscription/list`，国内区走 `https://open.bigmodel.cn`、海外区走 `https://api.z.ai`；团队版使用同一组端点的 team 参数。
...
- **数据源**：`https://api.commandcode.ai/internal/billing/credits` 与 `/internal/billing/subscriptions`（新账号另有 `/alpha/*` 一组端点）。
- **配额数据**：5 小时额度、每周额度、月度额度。
- **实测可行性**：优先使用 CLI 写入的 API Key；Web Session Cookie 仅作为回退路径。
...
- **数据源**：New API / OneAPI 规范的 `/api/status`、`/api/user/self`、`/api/usage/token/` 等端点。
- **配额数据**：账户剩余额度、已用额度、美元/人民币换算。
- **实测可行性**：完全可行且生态广泛。

### 17. 无法在 Hub 独立添加的本地服务商（仅 Kiro）
- **Kiro (`kiro`)**：唯一的取数方式是 spawn 本机 `kiro-cli chat --no-interactive /usage` 并解析 stdout，文件里没有任何 HTTP 端点或可粘贴的 token 字段，因此 Hub 侧无法完成；`limitProviderSources.js` 标为 `authority: 'unsupported'`，`HUB_ACCOUNT_PROVIDERS` 将其排除，这是正确的。
- **Cursor (`cursor`)** 与 **Grok (`grok`)** 早期同样被判为「仅本地」，但它们实际都走普通 HTTPS：Cursor 用 `WorkosCursorSessionToken`、Grok 用 CLI 存在 `~/.grok/auth.json` 里的 bearer token。两者现已分别通过 `cursorSessionToken` 与 `grokBearerToken` 在 Hub 放行。
...
经过对额度渲染代码（现为 `src/shared-ui/views/limits.js` 的 `renderLimits` / `renderLimitCards`，数据侧为 `src/shared-ui/core/data.js` 的 `limitCards`）的审查，当时网页端额度展示存在以下严重缺陷：

#### 缺陷 1：AGY 在网页端被“平铺打散”，丢失了模型分组（严重违背 AGY 规范）✅ 已修复（`views/limits.js` 的 `groupAntigravityWindows`）
...
#### 缺陷 2：无百分比的额度（Balance / Credits）展示错误与仪表盘缺失 ✅ 已修复（`isBalanceKind` 走金额主值路径）
- **现状逻辑**（原 `data.js:569-588` 与 `app.js:1492-1506`；对应逻辑现位于 `src/shared-ui/core/data.js` 的余额换算与 `src/shared-ui/views/limits.js` 的 `isBalanceKind`）：
...
#### 缺陷 3（部分修复：metric 提示与 resets 类型已对齐，其余需按当前代码复核）：Codex 的额外额度与重置额度展示不统一
...
> M-2（Codex localhost 回调引导）、M-4（AGY 分组）、M-5（余额型窗口）已随 shared UI 落地，
> 分别对应 `core/i18n.js` 的 `accounts.codexOauthLocalhostNotice`、`views/limits.js` 的
> `groupAntigravityWindows` 与 `isBalanceKind`。M-1 / M-3 / M-6 / M-7 需按当前代码逐条复核后再决定。

| 编号 | 模块 | 修改项名称 | 严重级别 | 修改内容与目标 | 涉及文件 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **M-1** | Accounts 表单 | **AGY 认证模式精简与文案修正** | 高 (P1) | 移除 Hub 网页端不切实际的“本地 127.0.0.1 RPC”模式入口，或明确标注“仅用于内网穿透/反代服务”。OAuth 步骤说明中强化“复制 4/0A... 授权码”的高亮指引。 | `src/shared-ui/views/accounts.js`<br>`src/shared-ui/core/i18n.js` |
| **M-2** | Accounts 表单 | **Codex OAuth 回调引导优化** | 高 (P1) | 针对 Codex OAuth 授权后重定向到 `localhost:1455` 导致浏览器“无法访问”的普遍痛点，在 Step 2 增加醒目的图文/黄色提示框，说明“浏览器显示打不开页面是正常的，请直接复制地址栏中的全部 URL 粘贴到下方即可完成验证”。 | `src/shared-ui/views/accounts.js`<br>`src/shared-ui/core/i18n.js` |
| **M-3** | Accounts 表单 | **MiMo / Kimi / Claude 凭据输入提示细化** | 中 (P2) | 1. MiMo 表单增加对 `userId` 与 `serviceToken` 的独立输入框或自动解析校验，避免整串 Cookie 拼错；<br>2. Kimi 明确区分 API Key（仅并发）与 Web Token（带 5h 额度条）的作用差异；<br>3. Claude 增加云端 Cloudflare 风险提示。 | `src/shared-ui/views/accounts.js`<br>`src/shared-ui/core/i18n.js` |
| **M-4** | Limits 页面 | **Antigravity 分组显示（Grouped Layout）重构** | 高 (P1) | 对齐 CodexBar 和桌面端实现：在网页端引入 `antigravityQuotaGroups` 逻辑。当渲染 AGY 卡片时，按 Gemini Pro / Gemini Flash / Claude 等模型分别创建分组框，组内并列展示 5-hour 与 Weekly 两个标准额度条，杜绝扁平打散。 | `src/shared-ui/views/limits.js`<br>`src/shared-ui/styles/app.css` |
| **M-5** | Limits 页面 | **金额/Credits 余额型窗口的视觉统一** | 高 (P1) | 针对 DeepSeek、OpenCode Zen、OpenRouter 等余额型数据：当无百分比或为金钱时，主值大字突出显示实际金额（如 `$12.50` 或 `￥88.00`），副文本展示消费细则（Today / Month Spend），不得强行拼装虚假或无意义的百分比。 | `src/shared-ui/views/limits.js`<br>`src/shared-ui/core/data.js`<br>`src/shared-ui/styles/app.css` |
| **M-6** | Limits 页面 | **Codex 专属额度与重置次数标准化对齐** | 中 (P2) | 确保 Codex 的主 5h 窗口、周窗口、Luna/Spark 专属额度、Reset Credits 各自以标准小卡片并列或专用通栏呈现，统一图标、标签风格及 Reset 倒计时格式。 | `src/shared-ui/views/limits.js` |
| **M-7** | Limits 页面 | **卡片头部信息与健康徽章规范化** | 中 (P2) | 统一各卡片头部：图标 + 账号展示名（支持工作区区分）+ 套餐徽章（Plus / Team / Tier 等）+ 刷新时间。状态徽章统一映射为 `正常`、`过期/需刷新`、`错误`、`已停用` 四类状态。 | `src/shared-ui/views/limits.js`<br>`src/shared-ui/core/data.js` |

---

### 总结结论
1. **获取可行性**：除必须 spawn 本地 CLI 的 Kiro 外，当前 Hub 支持的 26 家服务商**均真实具备额度或数据获取链路**。其中 Codex、AGY、OpenRouter、DeepSeek、OpenCode、Copilot、Z.ai、Volcengine、MiniMax 接口极其可靠；Claude、Qoder、MiMo、Ollama 因强依赖 Web Cookie，在云端环境存在不同程度的生命周期或 WAF 挑战。
...
3. **额度页展示统一**：当时网页端额度页最大的问题是**“一刀切的简单 2 列网格”**抹杀了复杂服务商的层次结构；AGY 分组与余额型窗口两条已修复，其余仍需继续。必须将 AGY 的模型分组体系、Codex 的核心窗口+专属额度体系、以及 DeepSeek/OpenRouter 的资金余额体系进行组件化规范，彻底对齐桌面端的精致布局与设计品味。
