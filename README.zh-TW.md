...
| <img src=".github/assets/tools-icon/claude-desktop.png" width="28" alt="Claude Desktop" /> | Claude Desktop | `<platform-app-data>/Claude/` 與 `Claude-3p/`（Local Agent / Cowork transcript） | ✅ | — | ✅ |
...
- **即時 Token 追蹤**：Claude Code、Codex、Cursor、GitHub Copilot、Antigravity、OpenCode 等 52+ 種 AI 工具，每輪對話後 UI 在數秒內更新（完整清單見上方表格）
- **單一 session 明細**：點進 Claude Code、Claude Desktop、Codex、OpenCode 或 Reasonix 的 session，
...
- **服務狀態面板**：在額度檢視的「額度健康」中追蹤 Claude、OpenAI、Cursor 與 DeepSeek status 頁，支援手動或定時重新檢查
...
單一裝置使用應用程式的本機模式。它直接讀取這台機器的本機資料，不需要 Hub 或 agent。
...
```text
模式 A——本地（預設，免設定）
    桌面應用程式 (Electron) ──▶ tokscale ──▶ ~/.claude、~/.codex、$HERMES_HOME

模式 B——同步（選用，多裝置）
    裝置 A agent ──▶
    裝置 B agent ──▶  hub  ──▶  任一裝置上的桌面應用程式或瀏覽器
    裝置 C agent ──▶
```
...
本專案由開發者在以下環境中進行開發與測試，**未經測試的平台或環境可能存在未知問題**，遇到問題歡迎提交 Issue 或 PR：

- **Docker Compose 中樞（Hub）**：測試通過，用於多裝置用量彙整與資料同步。
- **Windows 11**：日常開發與主要測試平台。
  - 重點測試開發工具：**Codex**、**OpenCode**。
