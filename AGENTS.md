...
- Release bodies are **Chinese-only**: `.github/RELEASE_TEMPLATE.md` carries one hand-written 本次更新 block between the `app-update-notes:zh` markers, and `scripts/generate-release-notes.js` renders the 快捷下载 list from its `RELEASE_ARTIFACTS` table plus the Hub image section.
