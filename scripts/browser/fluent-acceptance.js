...
await page.addInitScript(({stats})=>{
 let settings={language:'zh-CN',theme:'light',hubMode:'local',clients:'codex,claude',collectionMode:'live',collectionIntervalMs:300000,historyIntervalMs:900000,exportIntervalMs:60000,syncUploadIntervalMs:600000,systemGlass:true,reduceMotion:'system'};window.__patches=[];
 const noop=()=>()=>{};
 window.tokenMonitor={
  getAppInfo:async()=>({platform:'linux',version:'0.47.0-rev.4'}),getSettings:async()=>settings,updateSettings:async(patch)=>{window.__patches.push(patch);settings={...settings,...patch};return settings},prefsFromSettings:s=>s,prefsToSettingsPatch:p=>p,hasSecret:async()=>false,readFlag:()=>null,writeFlag(){},getCatalog:async()=>({collectionModeIntervals:[300000,600000,900000,1800000,3600000],historyIntervals:[300000,600000,900000,1800000,3600000],exportIntervals:[60000,300000,900000,3600000],syncUploadIntervals:[0,600000,1200000,1800000]}),onStatsPush:noop,onStreamStatus:callback=>{callback({connected:true,mode:'local'});return noop()},onRetry:noop,onSettingsPush:noop,getSnapshotMeta:async()=>null,getSyncHealth:async()=>null,
